...
class Worker(QtCore.QThread):
	...
	def startTrain(self,train_cmd_str):#render
		print("\n calling from thread ",train_cmd_str,"\n")
		self.strT = train_cmd_str
		self.start()
	...
	def run(self):
		strT = self.strT.split()
		print(strT)
		self.p = subprocess.Popen(strT)
...
# 
    def stop_cmd(self):
    	...
    	self.trainButton_relaunch.setDisabled(False)
    	self.cancelButton.setDisabled(False)
    	self.weightButton.setDisabled(False)
    	self.p.terminate()
    	# self.r.terminate()
# now
...
# .,.,.,.,.,.,.,.,.,
    ...
    def relaunch_cmd(self):
    	...
    	self.trainButton_relaunch.setDisabled(True)
    	self.cancelButton.setDisabled(True)
    	self.weightButton.setDisabled(True)

    	self.cal_weight()
    	relaunch_cmd_str = './darknet detector train voc_gui.data gui.cfg backup/gui.backup -gpus 1'
    	strR = relaunch_cmd_str.split()
    	self.p=subprocess.Popen(strR)
#>>>
    ...
    # def updateUi(self):
    # 	"""called when thread stop running"""
    # 	self.trainButton.setDisabled(False)
    # 	self.trainButton_relaunch.setDisabled(False)
    # 	self.cancelButton.setDisabled(False)
    # 	self.weightButton.setDisabled(False)

    def launchTraining(self):#makePicture
    	self.trainButton.setDisabled(True)
    	self.trainButton_relaunch.setDisabled(True)
    	self.cancelButton.setDisabled(True)
    	self.weightButton.setDisabled(True)
    	self.cal_weight()
    	train_cmd_str = './darknet detector train voc_gui.data gui.cfg -gpus 1'
    	strT = train_cmd_str.split()
    	self.p = subprocess.Popen(strT)
    	# if p.
...
#######_______________________________________________________________
        ...
        # self.thread.output.connect(self.train_cmd)
        self.trainButton.clicked.connect(self.launchTraining)
